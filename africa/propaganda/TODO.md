...
- [x] AI video generation via fal.ai (Kling: i2v, t2v, v2v, motion-control)
- [x] Face/character swap pipeline (first-frame + nano-banana + motion-control)
- [x] Person segmentation via BiRefNet v2 (fal.ai) + ffmpeg composite with fgScale
- [x] Subtitle generation from Whisper transcript → SRT → burn onto video
- [x] Are.na channel scraper (images + block metadata)
- [x] Cut detection + video splitting (ffmpeg scene filter)
- [x] Video remix (audio swap + text overlay, auto-wrapped)
