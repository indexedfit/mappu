...
export type FalMode = "image-to-video" | "text-to-video" | "video-to-video" | "motion-control";
...
export interface FalOptions {
  ...
  // motion-control: transfer motion from video onto character in image
  characterOrientation?: "video" | "image";  // "video" = match video orientation (up to 30s), "image" = match image (up to 10s)
  keepOriginalSound?: boolean;               // retain audio from reference video
...
const MODELS = {
  "2.6": {
    ...
    "motion-control": "fal-ai/kling-video/v2.6/pro/motion-control",
  },
  "3.0": {
...
export async function falGenerate(opts: FalOptions): Promise<FalResult> {
  ...
  switch (mode) {
    ...
    case "motion-control": {
      if (!imageUrl && !opts.imagePath) throw new Error("motion-control requires imagePath or imageUrl (character appearance)");
      if (!videoUrl && !opts.videoPath) throw new Error("motion-control requires videoPath or videoUrl (motion source)");
      ...
      const charOrientation = opts.characterOrientation || "video";
      console.log(`  [fal] motion-control: orientation="${charOrientation}", keepSound=${opts.keepOriginalSound ?? true}`);
